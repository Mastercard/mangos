...
run() {
    ...
    # Stop and reset previous swtpm if it exists (for 2nd and 3rd VM boots)
    systemctl --user stop "mangos-test-${testid}-swtpm" 2>/dev/null || true
    systemctl --user reset-failed "mangos-test-${testid}-swtpm" 2>/dev/null || true
